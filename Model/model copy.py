...
class KANLayer(nn.Module):
    ...
    Replaces traditional MLP with a non-linear function basis expansion.
    """
    def __init__(self, in_dim, out_dim, num_basis=8):
        super().__init__()
        # allow in_dim to be passed as int or single-element list/tuple
        ...
        # Learnable coefficients for sine basis functions
        # coeff: (out_dim, in_dim, num_basis)
        self.coeff = nn.Parameter(torch.randn(self.out_dim, self.in_dim, self.num_basis) * 0.1)

        # Per-channel frequencies for greater expressivity and stability: (in_dim, num_basis)
        # Initialize small to avoid extreme oscillations at start
        self.freq = nn.Parameter(torch.randn(self.in_dim, self.num_basis) * 0.05)

        # Optional phase shifts per-basis (broadcast across channels)
        self.phase = nn.Parameter(torch.zeros(self.num_basis))

        # Linear term for residual connection
        self.linear = nn.Linear(self.in_dim, self.out_dim)

    def forward(self, x):
        # Expect x with last dimension == in_dim: (..., in_dim)
        if x.shape[-1] != self.in_dim:
            raise ValueError(f"KANLayer expected input last-dim {self.in_dim}, got {x.shape[-1]}")

        # x.unsqueeze(-1): (..., in_dim, 1)
        # self.freq: (in_dim, num_basis) -> unsqueeze to (1, in_dim, num_basis) for broadcasting
        basis = torch.sin(x.unsqueeze(-1) * self.freq.unsqueeze(0) + self.phase.view(1, 1, -1))
        # basis: (..., in_dim, num_basis)

        # einsum over input channels and bases to produce (..., out_dim)
...
class FeedForward(nn.Module):
    """
    FeedForward layer using Kolmogorov–Arnold Network (KAN) expansion.
    """
    def __init__(self, dim, hidden_dim, dropout=0.):
        super().__init__()
        self.kan1 = KANLayer(dim, hidden_dim)
        self.kan2 = KANLayer(hidden_dim, dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        return self.dropout(self.kan2(torch.relu(self.kan1(x))))
